...
// Replace the total count only. Skip talk-id refs: doc-N, #tN, t-prefixed anchors,
// the refchip visible "#N title", and percentages — guarded by the look-arounds.
const bumpCount = (oldN, newN) => {
    const re = new RegExp(`(?<![#\\w-])${oldN}(?![\\d%])`, "g");
    const changed = [];
...
const main = async () => {
    ...
    if (added) {
        ...
        console.log(`\n+${added} note(s). Talk count ${startCount} → ${startCount + added}.`);
