...
## 用 Apache Doris 打造即時 AI 可觀測性與分析——現場實機演示
@ SelectDB
一場現場實機演示展示 Apache Doris 同時扮演三種角色:讓 AI 代理(Claude)撰寫程式並玩猜數字遊戲、其推理過程被記錄為可觀測性追蹤;由 Postgres CDC 驅動、搭配物化視圖與快速 join 的即時電商儀表板;以及單節點吞吐量基準測試,持續維持每秒 800 萬至 1000 萬列的寫入速度。
...
## 為什麼 AI 代理需要的是「資料挽具」，而不只是 Lakehouse
@ Will Martin, Dremio
主張代理式工作負載需要對話節奏、次秒級的資料存取，並提出由開放 catalog（治理、最佳化）、語意層（共享商業定義）與聯邦式查詢引擎（快取、跨來源存取）構成的「資料挽具」——全部建立在 Iceberg、Arrow 等開放標準之上，讓代理能取得可存取、可理解、高效能的資料，且不被廠商綁定。
